...
export async function POST(request: NextRequest) {
  ...
  const { name, email, phone, user_id, manager_id } = body
  ...
  if (profile.role === 'Manager') {
    // Manager só pode atribuir a subordinados
    ...
    // manager_id só pode ser ele mesmo
  ...
  const finalManagerId = manager_id || (profile.role === 'Manager' ? user.id : null)
