...
export default function ManagerContacts() {
  ...
  const [form, setForm] = useState({ name: '', email: '', phone: '', user_id: '' })
  ...
  useEffect(() => {
    const checkAndLoad = async () => {
      ...
      await Promise.all([
        ...
        loadSellers(session.access_token)
      ])
    }
  ...
  const loadSellers = async (token: string) => {
    const res = await fetch('/api/manager/list-sellers', {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (result.error) {
      ...
      alert(editingId ? 'Contato atualizado!' : 'Contato adicionado!')
      setForm({ name: '', email: '', phone: '', user_id: '' })
      setShowForm(false)
      setEditingId(null)
      loadContacts(session.access_token)
    }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-gray-300 text-lg">Carregando contatos...</div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-black p-6">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-10">
          <div>
            <h1 className="text-3xl font-light text-gray-100 mb-1">Contatos da Equipe</h1>
            <p className="text-gray-400 text-sm">Gerencie os contatos dos seus vendedores</p>
          </div>
          ...
          <div className="bg-gray-900 border border-gray-800 p-6 rounded-lg mb-8">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div>
                ...
                >
                  <option value="">Atribuir a mim (Gerente)</option>
                  {sellers.map(s => (
        ...
        {contacts.length === 0 ? (
          <div className="bg-gray-900 border border-gray-800 p-12 rounded-lg text-center">
            <p className="text-gray-400">Nenhum contato na equipe.</p>
          </div>
        ) : (
              ...
              <table className="w-full">
                <thead className="bg-gray-800 border-b border-gray-700">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Dono</th>
